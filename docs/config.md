...
* [`veritise-node config`](#veritise-node-config)
...
EXAMPLES
  $ veritise-node config -p bootstrap
  $ veritise-node config -p testnet -a dual --password 1234
  $ veritise-node config -p mainnet -a peer -c custom-preset.yml
  $ veritise-node config -p mainnet -a my-custom-assembly.yml -c custom-preset.yml
  $ veritise-node config -p my-custom-network.yml -a dual -c custom-preset.yml
